...
async def transaction_history(message: Message):
    ...
    async def ask_for_date(message: Message):
        await message.answer("Введіть дату у форматі MM-DD, з якої ви хочете побачити транзакції:")
    async def transaction_from_date(message: Message):
        ...
        try:
            filter_date = datetime.strptime(input_date, "%m-%d")
        except ValueError:
            await message.answer("Неправильний формат дати. Будь ласка, введіть у форматі MM-DD.")
            return


        filtered_transactions = [
            t for t in transactions if datetime.strptime(t["date"], "%m-%d") >= filter_date
        ]
